...
class VendasManager {
    ...
    filtrarVendas(termo) {
        if (!termo.trim()) {
            ...
            this.vendasFiltradas = this.vendas.filter(venda => {
                ...
                return cliente?.nome.toLowerCase().includes(termoLower) ||
    ...
    renderizarVendas(vendas = this.vendasFiltradas) {
        ...
        container.innerHTML = vendas.map(venda => this.criarCardVenda(venda)).join('');
                ...
                <div class="card border-0 bg-white p-3 h-100">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <span class="fw-bold text-primary">R$ ${this.formatarMoeda(venda.valorTotal)}</span>
                        </div>
                    </div>
    ...
    setupCardEventListeners() {
        ...
        document.querySelectorAll('.view-sale-btn').forEach(btn => {
            btn.addEventListener('click', (e) => this.visualizarVenda(e.target.dataset.saleId));
        });

        // Botões de editar
        document.querySelectorAll('.edit-sale-btn').forEach(btn => {
            btn.addEventListener('click', (e) => this.editarVenda(e.target.dataset.saleId));
        });
    }
    ...
    async criarVenda(event) {
        ...
        try {
            ...
            const offcanvas = bootstrap.Offcanvas.getInstance(document.getElementById('newSaleOffcanvas'));
            offcanvas.hide();
