...
function setActiveSidebarItem(currentPage) {
  ...
  const pageKey = currentPage.replace(".html", "").toLowerCase();
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  // Garantir que o item "Pagamentos" exista no sidebar (corrige casos de cache/versões diferentes)
  try {
    ensurePaymentsMenuExists();
  } catch (e) {
    console.warn("Falha ao garantir item Pagamentos no sidebar:", e);
  }
...
// Garante que o item Pagamentos exista no sidebar; idempotente
function ensurePaymentsMenuExists() {
  try {
    const sidebar = document.getElementById("sidebar");
    if (!sidebar) return;

    const existing = sidebar.querySelector('[data-page="pagamentos"]');
    if (existing) return; // já existe

    // Tentar inserir antes do item de relatórios, se existir
    const relatoriosItem = sidebar.querySelector('[data-page="relatorios"]');
    ...
    if (relatoriosItem && relatoriosItem.parentNode) {
      relatoriosItem.parentNode.insertBefore(wrapper, relatoriosItem);
    } else {
      // fallback: anexar ao final
      const container = sidebar.querySelector(".px-3") || sidebar;
      container.appendChild(wrapper);
    }
  } catch (e) {
    console.warn("Não foi possível garantir item Pagamentos no sidebar:", e);
  }
}
