...
class ClientesManager {
  ...
  renderizarClientes() {
    ...
    container.innerHTML = this.clientesFiltrados
      .map((cliente) => this.criarCardCliente(cliente))
      .join("");
  ...
  setupCardEventListeners() {
    document.querySelectorAll(".view-client-btn").forEach((btn) => {
      btn.addEventListener("click", (e) =>
        this.visualizarCliente(e.target.dataset.clientId)
      );
    });

    document.querySelectorAll(".edit-client-btn").forEach((btn) => {
      btn.addEventListener("click", (e) =>
        this.editarCliente(e.target.dataset.clientId)
      );
    });
  }
  ...
  preencherModalVisualizacao(cliente) {
    ...
    document.getElementById("viewClientLimit").textContent =
      this.formatarMoeda(limite);
    document.getElementById("viewClientDebt").textContent =
      this.formatarMoeda(divida);
    document.getElementById("viewClientAvailable").textContent =
  ...
  preencherModalEdicao(cliente) {
    ...
    document.getElementById("editClientLimit").value =
  ...
  async salvarEdicaoCliente() {
    ...
    const dadosCliente = {
      ...
      limiteCredito:
        parseFloat(document.getElementById("editClientLimit").value) || 0,
  ...
  async criarCliente(event) {
    ...
    const dadosCliente = {
      ...
      limiteCredito:
        parseFloat(form.querySelector('input[name="limite"]').value) || 0,
      prazoPagamentoPadraoDias: 30,
    };
    ...
    try {
      ...
      console.log("Disparando evento clienteCriado:", {
        nome: novoCliente.nome,
        id: novoCliente.id,
      });
      window.dispatchEvent(
        new CustomEvent("clienteCriado", {
          detail: { nome: novoCliente.nome, id: novoCliente.id },
        })
      );

      const offcanvas = bootstrap.Offcanvas.getInstance(
        document.getElementById("newClientOffcanvas")
      );
      offcanvas.hide();
